# linuxherd/ui/node_page.py
# NEW FILE: Page for managing Node.js versions via bundled NVM.
    ...
    def refresh_data(self):
        ...
        if self._cached_installed is not None and not self._is_first_load:
            ...
            try:
                ...
                self.installed_list.clear()
                if self._cached_installed: self.installed_list.addItems(self._cached_installed)
                else: self.installed_list.addItem("No versions installed (via LinuxHerd).")
            except Exception as e:
