...
import traceback # For debugging Popen errors
import tempfile # For temp logs if needed

# Import central config (even if not directly used yet, good practice)
try:
    from . import config
except ImportError:
    # Handle case where config might not be found during standalone test?
...
def _read_pid_file(pid_file_path_str):
    ...
    try:
        ...
        if not pid_str: return None # File is empty
        pid = int(pid_str)
        if pid <= 0: print(f"PM Warning: Invalid PID {pid} in {pid_file}."); return None
        return pid
    except (ValueError, IOError) as e: print(f"PM Warning: Failed reading PID from {pid_file}: {e}"); return None
    except Exception as e: print(f"PM Error: Unexpected reading PID file {pid_file}: {e}"); return None
...
def _check_pid_running(pid):
    ...
    try:
        ...
        return False # ESRCH (No such process) or EPERM (don't have permission - assume not running from user check)
    except Exception as e: # Catch other potential errors
         print(f"PM Warning: Unexpected error checking PID {pid}: {e}"); return False
    else:
        return True # Signal 0 succeeded, process exists and we can signal it
...
def start_process(process_id, command, pid_file_path, working_dir=None, env=None, log_file_path=None):
    ...
    try:
        ...
        if log_file_path: Path(log_file_path).parent.mkdir(parents=True, exist_ok=True)
        pid_path_obj.parent.mkdir(parents=True, exist_ok=True)

        # Remove stale PID file before starting
        pid_path_obj.unlink(missing_ok=True) # Use missing_ok=True

        effective_env = os.environ.copy()
        if env: effective_env.update(env)

        # Setup logging for the Popen call itself
        ...
        print(f"Process Manager: Starting '{process_id}' (PID file: {pid_file_path}). CMD: {' '.join(command)}")
        ...
        if initial_poll is not None:
            print(f"PM Warning: Initial process for '{process_id}' exited immediately (code: {initial_poll}). Daemon start likely failed.")
            ...
            if temp_log_used and log_handle:
                ...
                try:
                     ...
                     print("--- End temp log ---")
                except Exception as read_e: print(f"Could not read temp log: {read_e}")
        ...
        if process_id in running_processes: del running_processes[process_id] # Untrack if launch failed
        return False
    finally:
         if log_handle:
             try: log_handle.close()
             except Exception: pass
         # Decide whether to keep or remove temp log on success/failure
         # if temp_log_used and actual_log_path and Path(actual_log_path).exists():
         #     Path(actual_log_path).unlink()


def stop_process(process_id, signal_to_use=signal.SIGTERM, timeout=5):
    """Stops a managed process using its PID file and signals."""
    # (Implementation mostly unchanged - uses PID file)
    ...
    proc_info = running_processes[process_id]; pid_file = proc_info.get("pid_file")
    pid = _read_pid_file(pid_file)

    if not pid:
        print(f"PM: No PID for '{process_id}' in {pid_file}. Assuming stopped.");
        del running_processes[process_id]; return True
    if not _check_pid_running(pid):
        ...
        except OSError: pass; del running_processes[process_id]; return True

    print(f"PM: Stopping '{process_id}' (PID: {pid}) with {signal_to_use.name}...")
    try:
        os.kill(pid, signal_to_use); start_time = time.monotonic()
        while (time.monotonic() - start_time) < timeout:
            ...
            time.sleep(0.2)
        # Timeout, use KILL
        print(f"PM: Process '{process_id}' timeout. Sending SIGKILL."); os.kill(pid, signal.SIGKILL); time.sleep(0.5)
        if not _check_pid_running(pid):
             print(f"PM: Process '{process_id}' stopped after SIGKILL.");
             try: Path(pid_file).unlink(missing_ok=True)
             except OSError: pass; del running_processes[process_id]; return True
        else: print(f"PM Error: Process '{process_id}' did not stop after SIGKILL."); return False
    except Exception as e: print(f"PM Error: stopping '{process_id}' (PID: {pid}): {e}"); return False


def get_process_status(process_id):
    """Checks status of a managed process using its PID file."""
    # (Implementation unchanged)
    if process_id not in running_processes: return "stopped"
    proc_info = running_processes[process_id]; pid_file = proc_info.get("pid_file")
    pid = _read_pid_file(pid_file)
    if pid and _check_pid_running(pid): return "running"
    else: return "stopped" # Treat missing PID file or non-running PID as stopped
