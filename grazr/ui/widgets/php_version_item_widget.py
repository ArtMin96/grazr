...
try:
    ...
    print(
        "Warning: Could not import StatusIndicator from service_item_widget. Using dummy."
    )
    ...
    class StatusIndicator(QWidget):
        ...
        def set_color(self, color):
            try:
                ...
                print(f"Dummy StatusIndicator Error: {e}")
        ...
        def paintEvent(self, event):
            try:
                ...
                print(f"Dummy StatusIndicator Paint Error: {e}")
...
class PhpVersionItemWidget(QWidget):
    ...
    def __init__(self, php_version, initial_status="unknown", parent=None):
        ...
        main_layout.addWidget(
            self.status_indicator, 0, Qt.AlignmentFlag.AlignVCenter
        )

        # Version Info (Just the version number)
        self.name_label = QLabel(
            f"<b>{self.php_version}</b>"
    ...
    def update_status(self, status):
        ...
        try:
            ...
            print(
                f"DEBUG PhpVersionItemWidget: update_status called on deleted widget for {self.php_version}"
            )
        ...
        try:
            ...
            if hasattr(self, "configure_button") and self.configure_button:
                self.configure_button.setEnabled(
                    True
                )  # Configure always enabled? Yes.
                self.configure_button.update()
        except RuntimeError:
            print(
                f"DEBUG PhpVersionItemWidget: Child widget deleted during update_status for {self.php_version}"
            )
        except Exception as e:
            print(
                f"DEBUG PhpVersionItemWidget: Unexpected error during update_status children: {e}"
            )
    ...
    def set_controls_enabled(self, enabled):
        ...
        try:  # Wrap entire method for safety during shutdown
            if not enabled:
                ...
                if hasattr(self, "action_button") and self.action_button:
                    self.action_button.setEnabled(True)  # Basic enable
                if hasattr(self, "configure_button") and self.configure_button:
            ...
            print(
                f"DEBUG PhpVersionItemWidget: Widget deleted during set_controls_enabled({enabled}) for {self.php_version}"
            )
        except Exception as e:
            print(
                f"DEBUG PhpVersionItemWidget: Unexpected error in set_controls_enabled: {e}"
            )
