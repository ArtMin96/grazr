...
class SiteListItemWidget(QWidget):
    ...
    def __init__(self, site_info: dict, parent=None):
        ...
        self.domain_label.setFont(
            QFont("Sans Serif", 10, QFont.Weight.Medium)
        )  # Medium weight
    ...
    def _update_site_icon(self):
        ...
        icon_path = (
            ":/icons/secure.svg" if self.has_https else ":/icons/not-secure.svg"
        )
        try:
            icon = QIcon(icon_path)
            if not icon.isNull():
                self.site_icon_label.setPixmap(
                    icon.pixmap(self.site_icon_label.size())
                )
            else:
