    ...
    try:
        ...
        if not conf_file.is_file():
            ...
            try:
                ...
                # os.chmod can take Path objects directly
...
server {{
    ...
    access_log off;
    error_log "{error_log_path}" warn; # Still log errors for redirects
...
server {{
    ...
    https_server_block = ""
    if https_enabled:
        cert_path = get_cert_path(domain) # Returns Path object
        key_path = get_key_path(domain)   # Returns Path object

        if check_certificates_exist(domain): # Expects domain string
            # cert_path and key_path are already Path objects, .resolve() is good.
            # Direct use in f-string is fine.
...
server {{
    ...
    ssl_certificate "{cert_path.resolve()}";
    ssl_certificate_key "{key_path.resolve()}";
    ...
    {common_server_content}
            ...
            logger.warning(f"HTTPS enabled for {domain}, but certificate files missing. Site will only be served over HTTP.")
            # Revert HTTP block to serve content if certs are missing, as HTTPS block won't be created
            # This part of logic was already updated in previous step, ensure common_server_content is used
            http_server_block = rf"""
server {{
    listen 80;
    listen [::]:80;
...
def install_nginx_site(site_path_str):
    ...
    try:
        ...
        os.chmod(available_path, 0o644) # os.chmod takes Path object
        ...
        if enabled_path.is_symlink():
            # os.readlink returns string; available_path is Path object
            if Path(os.readlink(enabled_path)).resolve() != available_path.resolve():
                enabled_path.unlink()
                os.symlink(available_path, enabled_path) # os.symlink can take Path objects
